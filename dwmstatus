...
#
# This script will work almost completely in bash as well as zsh. The only
# component that is not transferable is the notification for emails which seems
# to use zsh specific regexes,
...
function testTimeNow(){
	...
	echo $2 "\t" $duration "\t" $(($bigSec%$duration))

	#if [ $(($bigSec%$duration)) -eq 0 ]; then
	current=$(($bigSec%$duration))
	if [[ $current -ge 0 && $current -lt 11 ]]; then
		return 0
	else
		return 1
	fi
...
function grd(){
	if testTimeNow 600 "greader"; then
...
if testTimeNow 120 "email"; then
		...
		if [ "$new" -ne 0 ]; then
			...
			things=$(echo $feed | sed -n -e 's/.*<title>\(.*\)<\/title>.*/\1/p' -e 's/.*<name>\(.*\)<\/name>.*/\t\1/p' | tail -n+2)
...
function pac(){
	if testTimeNow 120 "pacman"; then
		pup="$(pacman -Qqu | wc -l)"
		if [ "$pup" -ne 0 ]; then
			notify-send "Pacman has been run - $pup updates"
